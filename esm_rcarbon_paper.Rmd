...
library(rcarbon)
data(ewdates)
sites <- unique(data.frame(id=ewdates$SiteID,x=ewdates$Eastings,y=ewdates$Northings))
rownames(sites) <- sites$id
data(euroevol)
gb=subset(euroevol,Country=="Great Britain")
sites2 <- unique(data.frame(id=gb$SiteID,lon=gb$Longitude,lat=gb$Latitude))
sites2 <- subset(sites2,id%in%unique(ewdates$SiteID))
sp::coordinates(sites2) <- c("lon","lat")
sp::proj4string(sites2) <- sp::CRS("+proj=longlat +datum=WGS84")
sites3 <- spTransform(sites2, CRS("+init=epsg:27700"))
sites4=as.data.frame(sites3)
sites5=merge(ewdates,sites4,by.x="SiteID",by.y="id")
sites5$Eastings=sites5$lon
sites5$Northings=sites5$lat
sites5= sites5[,1:8]
ewdates =sites5
...
#Compute spatial weights (distances should be in km)
w <- spweights(d/1000,h=100)
# Define blocks
breaks <- seq(7000,5000,-500) #500 year blocks (try different values and single block)

# Spatial Permutation Test
res <- sptest(calDates=x,bins=bins1,timeRange=c(7000,5000),locations=sites,permute="locations",nsim=1000,breaks=breaks,spatialweights=w,ncores=1) 

par(mar=c(1,1,4,1),mfrow=c(1,2))
base=as(ewowin,"SpatialPolygons")
sp::proj4string(base) <- sp::CRS("+init=epsg:27700")

plot(base,col="antiquewhite3",border="antiquewhite3")
plot(res,index=2,option="raw",add=TRUE,legend=TRUE,legSize=0.7,location="topleft")

## Geometric Growth Rate for Transition 4
plot(base,col="antiquewhite3",border="antiquewhite3")
plot(res,index=2,option="test",add=TRUE,legend=TRUE,legSize=0.7,location="topleft")
