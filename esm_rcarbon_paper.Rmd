...
Once the package is downloaded and instaled, load _rcarbon_ by typing:
...
# Aggregate 
southlevant.spd.nnorm = spd(southlevant.x,timeRange=c(16000,9000),verbose=F)
sahara.spd.nnorm = spd(sahara.x,timeRange=c(16000,9000),verbose=F)
brazil.spd.nnorm = spd(brazil.x,timeRange=c(16000,9000),verbose=F)

southlevant.spd.norm = spd(southlevant.x,timeRange=c(16000,9000),verbose=F,datenormalised=T)
sahara.spd.norm = spd(sahara.x,timeRange=c(16000,9000),verbose=F,datenormalised=T)
brazil.spd.norm = spd(brazil.x,timeRange=c(16000,9000),verbose=F,datenormalised=T)


#plot
fmat=matrix(c(rep(1,3),rep(2,3),rep(3:5,each=2)),nrow=6,ncol=2)
layout(fmat,widths=c(0.4,0.8),heights=c(rep(1,6)))
...
par(mar=c(4,4,3,1))
plot(southlevant.spd.norm,fill.p='bisque3',ylim=c(0,0.4))
plot(southlevant.spd.nnorm,type = 'simple',add=TRUE)
title("c")

par(mar=c(4,4,3,1))
plot(sahara.spd.norm,fill.p='bisque3',ylim=c(0,0.4))
plot(sahara.spd.nnorm,type = 'simple',add=TRUE)
title("d")

par(mar=c(4,4,3,1))
plot(brazil.spd.nnorm,fill.p='bisque3',ylim=c(0,0.125))
plot(brazil.spd.norm,type = 'simple',add=TRUE)
title("e")
legend("topleft",legend=c("Normalised SPD","Not normalised SPD"),col=c('bisque3','black'),lwd=c(3,1))
```
