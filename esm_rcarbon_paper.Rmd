...
All analyses and figures require the installation of the _rcarbon_ package version <!-- specify version here --> and additional data supplied in the ESM. To install _rcarbon_ type the following command in the R console:
...
```{r}
# IntCal 13 Calibration Curve from Reimer et al 2013
...
alldatesLevant.emedyd <- calibrate(x=levant.emedyd$CRA, errors=levant.emedyd$Error, calCurves='intcal13', normalised=FALSE,verbose=FALSE)
...
sumCal <- calibrate(as.UncalGrid(sumUncal),eps=0,verbose=FALSE)

# Normalise Probability Sum to Unity
sumCal$PrDens <- sumCal$PrDens/sum(sumCal$PrDens)

# Calibrate dates without normalisation
x <- calibrate(d$CRA,d$Error,normalised=F,verbose=FALSE)
# Create SPD
SPDnn <- spd(x,timeRange=c(12000,8000),spdnormalised=T,verbose=FALSE)
...
Reimer, P. J., Bard, E., Bayliss, A., Beck, J. W., Blackwell, P. G., Ramsey, C. B., et al. (2013). [IntCal13 and Marine13 Radiocarbon Age Calibration Curves 0–50,000 Years cal BP.](https://doi.org/10.2458/azu_js_rc.55.16947) Radiocarbon, 55(4), 1869–1887. 
