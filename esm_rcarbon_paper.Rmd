...
All analyses and figures require the installation of the _rcarbon_ package version 1.3.0 and additional data supplied in the ESM. To install _rcarbon_ type the following command in the R console:
...
cctmp <- paste(system.file("extdata", package="rcarbon"), "/", "intcal13.14c", sep="")
cctmp <- readLines(cctmp, encoding="UTF-8")
cctmp <- cctmp[!grepl("[#]",cctmp)]
cctmp.con <- textConnection(cctmp)
intcal13 <- read.csv(cctmp.con, header=FALSE, stringsAsFactors=FALSE)
...
lines(intcal13[intcal13$BP<=14000 & intcal13$BP>=9000,"BP"], 
      reScale(intcal13[intcal13$BP<=14000 & intcal13$BP>=9000,"CRA"])*par('usr')[4],
      col="red",lwd=2)
legend("topleft",
       legend=c("Unnormalised SPD","Normalised SPD", "IntCal13","Areas with spikes"),
       col=c('bisque3','black',"red","orange"),
       lwd=c(5,1,1,5))
title("c",line=-2)
...
alldatesLevant.emedyd <- calibrate(x=levant.emedyd$CRA, 
                                   errors=levant.emedyd$Error, calCurves='intcal13', 
                                   normalised=FALSE,verbose=FALSE)
...
ew.mod <- modelTest(x.ew,errors=ewdates$C14SD,bins=bins.ew,timeRange=c(8000,4000),
                    nsim=1000,runm=50,ncores=ncores,backsight=100)
