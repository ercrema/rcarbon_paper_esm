...
sumUncalMat=matrix(NA,nrow=length(12000:5000),ncol=nrow(d))
for (i in 1:nrow(d))
{
 sumUncalMat[,i]=dnorm(12000:5000,mean=d$CRA[i],sd=d$Error[i])
}

PrDensNN = apply(sumUncalMat,1,sum)
PrDens = PrDensNN/sum(PrDensNN)
sumUncal=data.frame(CRA=12000:5000,PrDens=PrDens)

# Calibrate the entire uncalibrated SPD 
sumCal=calibrate.UncalGrid(sumUncal)
...
<!-- use the same study area as in figure 10? -->
